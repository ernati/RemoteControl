...
DWORD WINAPI ForwardThread(LPVOID lpParam) {
    ...
    while (true) {
        ...
        if (!recvn(sendSock, headerBuf.data(), headerSize)) break;
        // 2) payloadSize 파싱
        ...
        if (!recvn(sendSock, buf + headerSize, payloadSize)) { delete[] buf; break; }
        // 4) 수신자에게 전송
        if (sendn(recvSock, (char*)buf, (int)totalSize) == SOCKET_ERROR) { delete[] buf; break; }
        delete[] buf;
    }
